...
def paises(task_instance):
    ...
    for lista in links:
        if 'paises' in lista.lower():
            filespaises.append(lista)
    return filespaises

def versioning(task_instance):
    now = datetime.now()
    file = task_instance.xcom_pull(task_ids='Find_paises')
    objectname = file[0].split('/')[-1].replace('zip','CSV')
    bucket_name = 'pottencial-datalake-dev-raw'
    object_name = f"dados_publicos_cnpj/2022-2/Paises/{objectname}"
    try:
...
def extract(task_instance):
    ...
    folder = task_instance.xcom_pull(task_ids='Find_paises')[0].split('/')[-1].replace('.zip','')
...
def uploadS3(task_instance):
    # RECEBE O NOME DA PASTA
    file = task_instance.xcom_pull(task_ids='Find_paises')[0].split('/')[-1].replace('.zip','')
...
triggerdag = TriggerDagRunOperator(
    task_id="de_qualificacoes",
    trigger_dag_id="de_qualificacoes")

with DAG('de_paises', start_date=datetime(2022,12,16),
    ...
    taskdownloadpaises = PythonOperator(
        task_id = 'Find_paises',
        python_callable = paises
    )
...
taskgetLinks >> taskdownloadpaises >> taskversioning >> [taskdownload, taskskip] >> taskextract >> tasks3 >> fim >> trigger_task
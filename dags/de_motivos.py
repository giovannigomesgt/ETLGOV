...
def extract(task_instance):
    ...
    folder = task_instance.xcom_pull(task_ids='Find_Motivos')[0].split('/')[-1].replace('.zip','')
...
def uploadS3(task_instance):
    # RECEBE O NOME DA PASTA
    file = task_instance.xcom_pull(task_ids='Find_Motivos')[0].split('/')[-1].replace('.zip','')
...
triggerdag = TriggerDagRunOperator(
    task_id="de_municipios",
    trigger_dag_id="de_municipios")

with DAG('de_motivos', start_date=datetime(2022,12,16),
    ...
    taskdownloadcnaes = PythonOperator(
        task_id = 'Find_Motivos',
        python_callable = motivos
    )

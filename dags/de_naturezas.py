...
def versioning(task_instance):
    now = datetime.now()
    file = task_instance.xcom_pull(task_ids='Find_Naturezas')
    objectname = file[0].split('/')[-1].replace('zip','CSV')
    bucket_name = 'pottencial-datalake-dev-raw'
    object_name = f"dados_publicos_cnpj/2022-2/Naturezas/{objectname}"
    try:
...
def extract(task_instance):
    ...
    folder = task_instance.xcom_pull(task_ids='Find_Naturezas')[0].split('/')[-1].replace('.zip','')
...
def uploadS3(task_instance):
    # RECEBE O NOME DA PASTA
    file = task_instance.xcom_pull(task_ids='Find_Naturezas')[0].split('/')[-1].replace('.zip','')
...
triggerdag = TriggerDagRunOperator(
    task_id="de_paises",
    trigger_dag_id="de_paises")

with DAG('de_naturezas', start_date=datetime(2022,12,16),
    ...
    taskdownloadcnaes = PythonOperator(
        task_id = 'Find_Naturezas',
        python_callable = naturezas
    )

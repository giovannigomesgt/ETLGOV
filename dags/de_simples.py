...
def extract(task_instance):
    ...
    folder = task_instance.xcom_pull(task_ids='Find_Simples')[0].split('/')[-1].replace('.zip','')
...
def uploadS3(task_instance):
    # RECEBE O NOME DA PASTA
    file = task_instance.xcom_pull(task_ids='Find_Simples')[0].split('/')[-1].replace('.zip','')
...
triggerdag = TriggerDagRunOperator(
    task_id="de_empresas",
    trigger_dag_id="de_empresas")

with DAG('de_simples', start_date=datetime(2022,12,16),
    ...
    taskdownloadcnaes = PythonOperator(
        task_id = 'Find_Simples',
        python_callable = simples
    )
